<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="房间种类" width="200" align="center">
        <template slot-scope="{ row }">
          <span class="link-type">{{
            row.piece
          }}</span>
    ...
    <el-dialog title="创建琴房" :visible.sync="dialogFormVisible">
      <el-form
        ref="dataForm"
        :rules="rules"
        :model="temp"
      ...
      >
        ...
        <el-form-item label="种类" prop="piece">
          <el-input v-model="temp.piece" />
        </el-form-item>
      </el-form>
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        floor: [
          { required: true, message: '请填入楼层', trigger: 'blur' }
        ],
        name: [
          { required: true, message: '请填入房间编号', trigger: 'blur' }
        ],
        piece: [
          { required: true, message: '请填入琴房种类', trigger: 'blur' }
        ]
      },
  ...
  methods: {
    ...
    handleDelete(row, index) {
      ...
      this.$confirm('此操作将永久删除此琴房, 是否继续?', '提示', {
