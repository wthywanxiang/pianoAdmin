<template>
  <div class="app-container">
    ...
    >
      ...
      <el-table-column label="预约总时长(小时)" align="center" width="150px">
        <template slot-scope="{ row }">
          {{ (row.detail.lengthOfTime?(row.detail.lengthOfTime/10000):0) }}
        </template>
      </el-table-column>
      <el-table-column label="违约次数(次)" align="center" width="150px">
        <template slot-scope="{ row }"> {{ row.detail.violationCount?row.detail.violationCount:0 }} </template>
      </el-table-column>
      <el-table-column label="迟到次数（次）" align="center" width="150px">
        <template slot-scope="{ row }"> {{ row.detail.lateOfTime?row.detail.lateOfTime:0 }}  </template>
      </el-table-column>
      <el-table-column
        label="操作"
        align="center"
  ...
  data() {
    return {
      ...
      rules: {
        ...
        clazz: [
          { required: true, message: '请填入班级', trigger: 'blur' }
        ],
        name: [
  ...
  methods: {
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          addStudent({
            sno: this.temp.number,
            name: this.temp.name,
            clazz: this.temp.clazz,
